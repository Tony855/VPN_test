...
PUBLIC_IP_FILE="$CONFIG_DIR/public_ips.txt"     # IPv4公网地址池
PUBLIC_IP6_FILE="$CONFIG_DIR/public_ip6s.txt"  # IPv6公网地址池
...
[Interface]
Address = $gateway_ip4/$(echo "$subnet4" | cut -d'/' -f2)
PrivateKey = $server_private
ListenPort = $port

# IPv6 NAT规则（出口流量通过IPv6）
PostUp = ip6tables -t nat -A POSTROUTING -s $subnet4 -o $ext_if -j SNAT --to-source $public_ip6
PostDown = ip6tables -t nat -D POSTROUTING -s $subnet4 -o $ext_if -j SNAT --to-source $public_ip6
EOF
    ...
    if systemctl enable --now "wg-quick@$iface" &>/dev/null; then
        echo "接口 $iface 创建成功！"
        echo "客户端连接地址: $public_ip4"
        echo "IPv6出口地址: $public_ip6"
...
add_client() {
    ...
    # 提取配置信息
    subnet4=$(grep 'Address' "$CONFIG_DIR/$iface.conf" | awk '{print $3}')
    public_ip4=$(get_available_ip "$PUBLIC_IP_FILE" "$USED_IP_FILE" | head -1)
...
Address = $client_ip4/$(echo "$subnet4" | cut -d'/' -f2)
DNS = 8.8.8.8, 2001:4860:4860::8888

[Peer]
PublicKey = $(wg pubkey < <(grep 'PrivateKey' "$CONFIG_DIR/$iface.conf" | awk '{print $3}'))
PresharedKey = $client_preshared
Endpoint = $public_ip4:$(grep ListenPort "$CONFIG_DIR/$iface.conf" | awk '{print $3}')
AllowedIPs = 0.0.0.0/0, ::/0  # 同时路由IPv4和IPv6流量
